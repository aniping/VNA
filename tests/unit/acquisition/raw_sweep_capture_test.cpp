...
TEST(RawSweepCaptureTest, EmitsOrderedRangesThenReturnsCompletePayload) {
    ...
    EXPECT_EQ(*payload, validPayload(7));
    ASSERT_EQ(requests.size(), 4U);
    EXPECT_EQ(requests[0], (RawSweepChunkRequest{SweepId{41}, 7, 1, 0, 2}));
    EXPECT_EQ(requests[1], (RawSweepChunkRequest{SweepId{41}, 7, 1, 2, 1}));
    EXPECT_EQ(requests[2], (RawSweepChunkRequest{SweepId{41}, 7, 2, 0, 2}));
    EXPECT_EQ(requests[3], (RawSweepChunkRequest{SweepId{41}, 7, 2, 2, 1}));
