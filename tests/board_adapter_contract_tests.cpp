...
void rejected_prepare_returns_every_input_and_never_calls_back() {
...
void stale_prepare_authorization_is_rejected_without_side_effects() {
    using namespace vna::board;
...
void invalid_sweep_intent_is_rejected_before_prepare_is_accepted() {
    using namespace vna::board;
...
void accepted_prepare_is_non_inline_and_has_one_terminal() {
    using namespace vna::board;
...
void accepted_run_emits_deterministic_waves_and_one_terminal() {
    using namespace vna::board;
...
void rejected_run_returns_token_grant_and_sink_without_callbacks() {
    using namespace vna::board;
...
int main() {
    ...
    failures += vna::test::run(
        "rejected prepare returns every input and never calls back",
        rejected_prepare_returns_every_input_and_never_calls_back);
    failures += vna::test::run(
        "stale prepare authorization is rejected without side effects",
        stale_prepare_authorization_is_rejected_without_side_effects);
    failures += vna::test::run(
        "invalid sweep intent is rejected before prepare is accepted",
        invalid_sweep_intent_is_rejected_before_prepare_is_accepted);
    failures += vna::test::run(
        "accepted prepare is non-inline and has one terminal",
        accepted_prepare_is_non_inline_and_has_one_terminal);
    failures += vna::test::run(
        "accepted run emits deterministic waves and one terminal",
        accepted_run_emits_deterministic_waves_and_one_terminal);
    failures += vna::test::run(
        "rejected run returns token grant and sink without callbacks",
        rejected_run_returns_token_grant_and_sink_without_callbacks);
    return failures;
}
