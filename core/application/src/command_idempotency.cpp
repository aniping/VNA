...
bool samePayload(
    const UpdateChannelSweepControlCommand& left,
    const UpdateChannelSweepControlCommand& right) noexcept {
    return left.channelId == right.channelId && left.mode == right.mode &&
        left.sweepCount == right.sweepCount;
}
