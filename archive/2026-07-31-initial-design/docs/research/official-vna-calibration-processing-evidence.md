...
| **REF-04** Reference Plane provenance（`NET-04`） | **[E1]** 三家都用 calibration plane、extended plane、fixture/DUT plane 解释结果的物理位置。 | **[E2]** 厂商 UI 未提供统一可交换的 plane identity/schema。 | **[E3]** 每个网络处理节点在内部保存 `input_plane_id[]`、`output_plane_id[]` 和 plane transform revision；Web/SCPI 只查询同一 provenance chain 的稳定 plane/snapshot 身份和业务属性，不返回 transform revision。它是本项目可靠性设计，不冒充厂商内部对象。 | **[E4]** 实际 adapter/cable/fixture 资产与 plane 名称由现场配置确认。 |
...
| **MTH-09** Statistics 显示与查询（`MATH-09`） | **[E1]** Keysight/CMT 均能显示统计结果并通过命令启停/选择范围；成熟远程控制需要读回与 UI 同源。 | **[E2]** 可查询字段、范围寄存器、RMS/min/max 和 marker 复用方式不同。 | **[E3]** 内部 `StatisticsResultSnapshot` 保存 input revision；Web/SCPI 查询同一快照时只返回稳定结果 ID、values、count、range 和 validity，不返回内部 revision；不存在结果时返回状态而非零值。 | **[E4]** 无硬件事实。 |
