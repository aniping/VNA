...
外部 Command 或 Query 被系统纳入统一接受顺序时，对目标仪器权威状态取得的一致视图。Web 与 SCPI 不提交、接收或比较内部 revision；普通配置使用窄字段/稳定行 ID patch，在唯一 Control Executor 中按接受顺序处理，同一字段以后接受且成功提交的修改为准，普通编辑不取得 edit lease。启动扫频时系统在该切面解析并冻结当前 Channel 配置，之后的修改只影响后续 Logical Sweep。
