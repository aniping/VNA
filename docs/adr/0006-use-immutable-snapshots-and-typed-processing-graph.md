...
非 C 层 receiver/ratio/corrected/ProcessedNetwork 结果如果尚未存在，由 `MaterializeMeasurementStageOperation` 从明确 A/B 父对象惰性产生 `MeasurementStageSnapshot`；它不携带 AnalysisTrace、Marker 或 Limit revision。只有逐 Trace pipeline/projection 才进入 `EvaluateTraceOperation` 和 C 层 `AnalysisPublication`，因此 SCPI raw/corrected query 与 Touchstone/全矩阵导出不需要伪造 Trace。
