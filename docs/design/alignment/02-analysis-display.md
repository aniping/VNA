...
| ANA-03 | Trace 求值快照 | 一次读数必须对应已经完成的一致输入和确切分析设置；厂商未公开内部快照结构。 | TraceEvaluationSnapshot 不可变并带 provenance；Trace/Marker/Limit 在私有 batch 验证闭包后原子发布。Measurement Pipeline 求值入口显式接收 `ExecutionContext{stop_token, monotonic_deadline, BudgetHandle, ProgressSink}`，协作节点有界检查取消/deadline，不可中断节点进入隔离 lane/Drain；这是 E3 并发/审计设计。 | Core | Measurement Pipeline / SnapshotCatalog | `AnalysisInputRefSet + AnalysisTraceRevision + ExecutionContext → EvaluateTraceOperation → AnalysisPublication`；Live/Frozen/Imported/Derived 及 Memory/Accumulator supplemental refs 均有类型。 | 输入快照被回收、多输入代次不一致、单条 child 处理失败、定义在求值中改变、超时后不可中断节点仍运行；不得暴露半套 C 或反向拖垮成功父快照，也不得提前释放其预算。 | 每份 Trace 结果能反查全部 typed input、revision 和质量；stop/deadline 有界响应与隔离 Drain 容量持有；Live 三层失败隔离；Derived generation、Accumulator clear 与 batch commit 故障注入。 | 已由证据定案 |
...
| ANA-12 | Preview 视觉状态 | 用户能区分渐进结果、last-good 和新正式结果 | provisional overlay + sweep progress；正式发布一次替换同代 Trace/Marker/Limit | Core | Web View Model | Preview 运行 streaming-safe 子图，不能提升为正式求值 | 非 streaming 节点显示 last-good 或明确 unavailable | 扫描中、失败、取消、完成四种视觉状态验收 | 已明确 |
