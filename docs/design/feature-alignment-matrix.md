...
本文是候选整体架构与商用 VNA 功能之间的逐项核对入口。它不以“文档提到过某个名词”作为覆盖完成，而要求每个功能项同时明确外部行为、产品等级、领域落点、控制入口、数据/状态语义、异常边界和可执行验收证据。功能的定义、执行、正式事实与呈现位置统一服从 [六层职责模型](layered-architecture.md)；跨功能的 Buffer 所有权、A/B/Stage/C 父引用、校准、Diagram、Query/Export 与失败隔离统一服从 [端到端数据流与生命周期契约](data-flow.md)。
