...
| 连续扫描时直接读取可能混轮 | R4 明确 continuous sweep 的 measurement values 可能来自不同 sweeps，且 `INIT;*OPC?` 不代表 continuous sweep end。 | **E2**：R&S 明确警告，其他来源没有同等措辞。 | 正式网络查询固定 B 层 `measurement_snapshot_id`，Trace/Marker/Limit 查询固定 C 层 `analysis_publication_id`；下一轮完成不改变正在传输的响应。 |
...
Real、Mock、Replay Adapter 共享同一逻辑接口：`describe`、`prepare(BoardCallId, ...)`、`request_prepare_abort`、`start`、`request_abort`、`enter_safe_state`、`health`、`recover`。采集输出带 Manifest ID、generation、sub-sweep/source-state ID、axis range、receiver path、sequence、quality 和唯一 terminal；prepare abort 仅表示请求，唯一 prepare terminal 来自受监控 job 返回。任何 ADC/IQ、寄存器和厂商 SDK 结构都留在 Real Adapter 内。

### 7. Preview 与正式查询走不同 QoS

采集线程写固定池和有界内部队列；Preview broker 可以抽样、合并或让客户端 resync，慢浏览器不得占有底软 Buffer Lease。B/C 层正式结果原子发布后，Web 与 SCPI 都通过 Snapshot Catalog 读取；查询开始时 pin `measurement_snapshot_id` 或 `analysis_publication_id`，直到传输完成。
