...
M5 的最小 SCPI 垂直切片首先提供 raw TCP socket，默认端口为 5025。该方式不要求
安装 VISA，适合直接验证 Windows/MinGW GCC 与 Linux/GCC 的一致行为。第一阶段
默认只绑定 `127.0.0.1`；开放远程监听属于部署与安全配置，不由解析器隐式决定。

每条 TCP 连接拥有独立的 SCPI Session，并遵守以下消息边界：

- 客户端命令以 LF 结束，同一行可用分号组合多个命令头。
- 查询响应以 LF 结束；响应必须按接收顺序进入该会话的输出队列。
- `*OPC?` 和 `*WAI` 等同步命令等待 Operation 的确定状态，不等待固定时长。
- 二进制块、VXI-11、HiSLIP、USB/GPIB 与 VISA 留到后续兼容阶段。

Socket 接入层只负责连接、字节流和会话生命周期。命令树、错误队列、状态寄存器
和内部 Command/Query 映射均位于可脱离传输单独测试的 SCPI 模块中。
